...
def hill(message, key, decrypt = False):
    ...
    print "Keymatrix before is ",keymatrix
    keymatrix = matrix(keymatrix).round().T
    print "Key matrix is ",keymatrix
    if decrypt:
...
cv2.imwrite('15IT245_M1_stego_panda.png',merge_img)
...
print "l_encr_words_final is ",l_encr_words_final
...
decipherText = hill(enc_msg, key, True)
...
# for i in range(rows):
# 	for j in range(col):
# 		mse= mse+((img[i][j]-merge_img[i][j])**(2.0))

# mse_val=(mse[0]+mse[1]+mse[2])/(rows*col*1.0)
# print "The MSE value is: ",mse_val  

def mse_fun(imageA, imageB):
...
psnr = (math.log(((255)**(2.0))/(mse*1.0),10))*10
...
#Calculating Entropy
# entropy=0
# grey_img = cv2.imread('15IT245_M1_panda.png',0)
# hist = cv2.calcHist([grey_img],[0],None,[256],[0,256])
# print hist
# for i in range(256):
# 	p=hist[i]/(rows*col*1.0)
# 	entropy = entropy + (p*math.log(p,10))
# entropy = entropy*(-1)
# print "The entropy value is:", entropy
